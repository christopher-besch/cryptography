...
#include "transform/transform_decrypt.h"
#include "xor/xor_decrypt.h"
...
// when the score of a certain decryption is at least ((1 - <MAX_ALLOWED_DEVIATION>) * <score of best decryption>), it gets printed as well with hold_amount < 0
...
std::string do_decryptions(const std::string& str, const LibrarySearch& dictionary, const ConsoleArguments& console_arguments)
{
    ...
    else
    {
        ...
        // when start idx is negative <- hold_amount is too big
